...
1. Cliquer droit sur le fichier "memories_history.html" et copier le chemin vers le fichier, et mettre ce chemin dans le fichier `params.json` au niveau de la variable `memories_history_path`. Les chemins ne doivent être écrits qu'avec des slashes (`path/to/file`) ou des double anti-slashes (`path\\to\\file`) mais pas de simples anti-slashes.
